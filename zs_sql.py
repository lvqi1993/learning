...
def find_table_by_index(index,zsindex,ID):
    table,field = "",''
    select_sql = "select customer_name,{0} as indicatorInfo from cust_base_state where id = '{1}'"
    if index==None:
        ...
        for d in table_dic:
            if d.get(index):
                ...
                if i ==0:
                    ...
                    print('指标1'+select_sql)
                    return
                elif i==1:
                    select_sql = "select t1.customer_name,t2.{0} as time,t2.{1} as indicatorInfo from cust_base_state  t1,{2}  t2 where t1.id = '{3}' and  t2.id = '{4}'"
                    table = sql_table[i]
                    select_sql=select_sql.format('time',field,table,ID,ID)
                    print('指标2'+select_sql)
                    return
                # elif i ==2:
