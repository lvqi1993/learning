        ...
        if index[0]!="":
            ...
            if symbol and value:
                ...
                if len(time_)==1:
                    ...
                    sql = "select  LEFT(ts_code,6) as COMPANY_SYMBOL,  name as companyName  from companylist  where  LEFT(ts_code,6) in  ("+sql+")"
                    print(index[0]+"&"+sql)
                    return
                if len(time_)==2:
                    sql = sql+ " and (reportdate between date_format('{0}', '%Y-%m-%d') and date_format('{1}', '%Y-%m-%d'))".format(time_[0],time_[1])
                    sql = "select  LEFT(ts_code,6) as COMPANY_SYMBOL,  name as companyName from companylist where  LEFT(ts_code,6) in  ("+sql+")"
                    print(index[0]+"&"+sql)
                    return
                else:
                ...
                sql = "select DISTINCT t1.company_symbol ,t1.reportdate,t2.name as companyName ,t1.{0} AS companyValue from {1} t1,companylist t2"
                sql= sql.format(name,table)
                if len(time_)==1:
                    sql = sql+ " where reportdata = '{}'".format(time_[0])+" and SUBSTRING(t2.ts_code, 1, 6)=t1.company_symbol limit 100"
                    print(index[0]+"&"+sql)
                    return
                elif len(time_)==2:
                    sql = sql+ " where reportdate between date_format('{0}', '%Y-%m-%d') and date_format('{1}', '%Y-%m-%d')".format(time_[0],time_[1])+"  and SUBSTRING(t2.ts_code, 1, 6)=t1.company_symbol limit 100"
                    print(index[0]+"&"+sql)
                    return
                else:
                    ...
                    if flag:
                        for i in ASC:
                            if i in index:
                                sql = sql +" ORDER BY {0} DESC".format(name)
                                sql = sql +" limit 200"
                                print(index[0]+"&"+sql)
                                flag = 0
                                break
                    if flag:
                        for i in DESC:
                            if i in index:
                                sql = sql +" ORDER BY {0} ASC".format(name)
                                sql = sql +" limit 200"
                                print(index[0]+"&"+sql)
                                flag = 0
                                break
