...
def process_ntime_ncmp_order(sql,index,name):
    ASC = ['最高', "靠前", '最好', '最多', '大于', '超过', '前']
    DESC = ['最低', '靠后', '最差', '最少', '少于', '差']
    flag = 1
    if flag:
...
def main():
    ...
    if company!=None and time_!=[]:
        ...
        if index[0]!="":
            ...
            value = index[-1]
            if symbol and value:
                sql = "select DISTINCT company_symbol from {0} where ".format(table)
                sql=sql+name+symbol+value
                if len(time_)==1:
                    sql = sql+ " and reportdata = '{}'".format(time_[0])
                    sql = "select  LEFT(t1.ts_code,6) as COMPANY_SYMBOL,  t1.name as companyName ,t2.{0} as companyValue,t2.reportdate  from companylist t1 ,{1} t2  where SUBSTRING(t1.ts_code, 1, 6)=t2.company_symbol and  LEFT(t1.ts_code,6) in  ("+sql+") limit 100"
                    sql =sql.format(name,table)
                    print(index[0]+"&"+sql)
                    return
                if len(time_)==2:
                    sql = sql+ " and (t2.reportdate between date_format('{0}', '%Y-%m-%d') and date_format('{1}', '%Y-%m-%d'))".format(time_[0],time_[1])
                    sql = "select  LEFT(t1.ts_code,6) as COMPANY_SYMBOL,  t1.name as companyName ,t2.{0} as companyValue,t2.reportdate  from companylist t1 ,{1} t2  where SUBSTRING(t1.ts_code, 1, 6)=t2.company_symbol and  LEFT(t1.ts_code,6) in  ("+sql+") limit 100"
                    sql = sql.format(name, table)
                    print(index[0]+"&"+sql)
                    return
                ...
                sql = "select DISTINCT SUBSTRING(t2.ts_code, 1, 6) as company_symbol ,t1.reportdate,t2.name as companyName ,t1.{0} AS companyValue from {1} t1,companylist t2"
                sql = sql.format(name, table)
                res = process_asc_and_desc(index)
                if len(time_)==1:
                    sql = sql+ " where reportdata = '{}'".format(time_[0])+" and SUBSTRING(t2.ts_code, 1, 6)=t1.company_symbol order by t1.{0} {1}limit 100".format(name,res)
                    print(index[0]+"&"+sql)
                    return
                elif len(time_)==2:
                    ...
                    process_ntime_ncmp_order(sql, index, name)
        else:
            print('')
            return
            pass
